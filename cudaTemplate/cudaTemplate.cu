...
{
    int dim_grid_x, dim_grid_y, dim_grid_z;	// grid  dimensions
    int dim_block_x, dim_block_y, dim_block_z;	// block dimensions
    ...
    printf("Running configuration: \t %d threads\n\t\t\t grid of %d x %d x %d\n"
           "\t\t\t blocks of %d x %d x %d threads (%d threads with %d bytes of shared memory per block)\n", 
           nPos, dim_grid_x, dim_grid_y, dim_grid_z, dim_block_x, dim_block_y, dim_block_z, dim_block_x * dim_block_y * dim_block_z, shared_mem_size);
